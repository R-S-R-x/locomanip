...
class WBCCommandsCfg:
    """Command specifications for B2-PIPER WBC."""

    ee_pose = mdp.command_cfg.UniformPoseWBCCommandCfg(
        asset_name="robot",

        # 重要：
        # 这里必须和 b2_piper.usda 里的末端 body/link 名一致。
        # 如果 smoke test 报 body_name not found，就先用 findstr 查真实名字。
...
class B2PiperWBCEnvCfg(LeggedManipLabEnvCfg):
    """B2-PIPER whole-body-control environment config."""
    ...
    def __post_init__(self):
        ...
        # Actions
        # ---------------------------------------------------------------------
        # WBC 阶段先保持 B2-Z1 的动作尺度逻辑，但换成 B2-PIPER 资产。
        # 如果后续 action joint_names 没有在父类中正确覆盖，需要在这里显式设置：
        #
        # self.actions.joint_pos.joint_names = B2PIPER_JOINT_NAMES
        #
        # 先不强行写，避免和原项目的 action term 结构不一致。
        self.actions.joint_pos.scale = 0.25
        self.actions.joint_pos.clip = {".*": (-10.0, 10.0)}

        # 如果你的父类 action 配置支持这些字段，建议 smoke test 后再打开：
        # self.actions.joint_pos.joint_names = B2PIPER_JOINT_NAMES
...
class B2PiperWBCEnvCfg_PLAY(B2PiperWBCEnvCfg):
    ...
    def __post_init__(self) -> None:
        ...
        # use wider command ranges for play
        self.commands.base_velocity.ranges = self.commands.base_velocity.limit_ranges
        self.commands.ee_pose.ranges = self.commands.ee_pose.limit_rangesv